...
size_t sum_aligned(size_t num_gaps) {
    ...
    for (size_t i = 1; i <= num_gaps; i++) {
        ...
        if (i % ALIGNMENT == 0) {
            ASSERT((size_t)ceil((double)i / ALIGNMENT) ==
                   (size_t)ceil((double)(i - 1) / ALIGNMENT));
        }
    }
...
int inverse_grid_test() {
    ...
    for (size_t j = PAGE_SIZE; j <= STORAGE_SIZE_TESTING; j *= 2) {
        ...
        // Measure maximum number of gaps. The first gap has to be strictly
        ...
        for (size_t i = 0; i < num_gaps - 1; i++) {
            ...
            pr_info("%zu", i);
            ASSERT(barrier == (uint8_t *)anchor + sizeof(struct seg_head_s) +
                                  round_up(helper_size, ALIGNMENT) +
                                  sizeof(struct seg_tail_s) +
