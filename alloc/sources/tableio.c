...
size_t get_gap_size(const uint8_t *m_addr, size_t target) {
    ...
    for (size_t i = 0; i < target; i++) {
        if ((m_addr + i) >= g_mem_end ||
            read_map_value(m_addr + i) != UNALLLOCATED) {
            return i;
        }
    }
    return target;
...
bool is_segment_beginning(const uint8_t *m_addr) {
    return read_map_value(m_addr) == ALLOCATED_START;
}

bool is_gap_beginning(const uint8_t *m_addr) {
    if (read_map_value(m_addr) != UNALLLOCATED) {
    ...
    if (read_map_value(m_addr - 1) == UNALLLOCATED) {
...
static uint8_t read_map_value_helper(const uint8_t *p_addr, bool low) {
    ...
    pr_info("Reading %s nibble %x at %p", low ? "Low" : "High", n, p_addr);
    return n;
}
